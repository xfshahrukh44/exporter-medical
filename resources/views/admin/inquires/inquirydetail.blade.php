                        ...
                        <div class="card-body">
                            <table class="table table-striped table-bordered">
                                <tbody>
                                    ...
                                    <tr><th> {{ ($inquiry->extra_content == "Pay Online") ? 'Description': 
                                    'Subject' }} </th><td> {{ $inquiry->extra_content }} </td></tr>
                                    @endif
                                    ...
                                    <tr><th> Order Notes </th><td> {{ $inquiry->notes }} </td></tr>
                                    @endif
                                    @if( $inquiry->invoice != null)
                                    <tr><th> Recieve Invoice via Email </th><td> {{ ($inquiry->invoice == 1)? 'Yes' : 'No' }} </td></tr>
                                    @endif 
                                    @if( $inquiry->address != null)
                                    <tr><th> Address </th><td> {{ $inquiry->address }} </td></tr>
                                    @endif
                                    @if( $inquiry->amount != null)
                                    <tr><th> Amount </th><td> {{ $inquiry->amount }} </td></tr>
                                    @endif
                                    @if( $inquiry->link != null)
                                    <tr><th> Product Link </th><td> <a href="{{ $inquiry->link }}" target="_blank">Click Here</a> </td></tr>
                                    @endif
