...
Route::get('test',function(){
    // dd(Carbon::today()->setTimezone('America/New_York')->dayOfWeek);
   if(Carbon::today()->setTimezone('America/New_York')->dayOfWeek == 0)
        {
            $shipDate = Carbon::tommorrow()->setTimezone('America/New_York');
        }
        else if(Carbon::today()->setTimezone('America/New_York')->dayOfWeek == 6)
        {
            $shipDate = Carbon::now()->addDays(2)->setTimezone('America/New_York');
        }
        else{
            $shipDate = Carbon::now()->setTimezone('America/New_York');
        }
    dd($shipDate->format('Y-m-d\TH:i:s l'));
});
...
Route::get('uploadpro','HomeController@upload')->name('upload');
Route::get('storepro','HomeController@store')->name('store');
...
Route::get('pages/{name}', 'HomeController@getPages')->name('pages');
...
Route::post('querysubmit','HomeController@querysubmit')->name('querysubmit');
Route::post('aboutUsSubmit','HomeController@aboutUsSubmit')->name('aboutUsSubmit');
...
Route::get('store/{category?}/{name?}','ProductController@shop')->name('shop');
Route::get('filter/{method}/{category?}/{id?}/{slug?}','ProductController@sort')->name('sort');

Route::get('detail/{id}/{name}','ProductController@shopDetail')->name('shopDetail');
Route::get('category/{id}/{name?}','ProductController@categoryDetail')->name('categoryDetail');
Route::get('subcat/{id}/{name?}','ProductController@subcategoryDetail')->name('subcategory');
...
Route::get('/clear/cart', 'ProductController@clearCart')->name('clear.cart');
...
Route::post('/services', 'OrderController@services')->name('shipping');
Route::post('/ups-services', 'OrderController@upsservices')->name('upsservices');
Route::post('/dhl-services', 'OrderController@dhlservices')->name('dhlservices');
...
//verify-coupon
Route::get('verify-coupon/{code}', function ($code) {
    if (!$coupon = \App\Models\Coupon::where('code', $code)->first()) {
        return json_encode([
            'success' => false,
            'data' => [],
